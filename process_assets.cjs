...
const outputDirs = [
    ...
    'public/media/starting11/backgrounds',
    'public/media/teams',
    'public/media/logos',
    'public/media/icons',
    'public/documents',
...
function processImages(inputDir, outputDir, width = 800) {
    ...
    for (const file of files) {
        ...
        try {
            execSync(
                `cwebp -resize ${width} 0 -q 80 "${fullPath}" -o "${outputPath}"`,
                { stdio: 'ignore' },
            );
        } catch (e) {
...
processImages(
    'lead/Képek/Étel/Hét különlegességei',
    'public/media/food/specials',
    800,
);
...
processImages(
    'lead/Képek/Ital/Ital specialitások',
    'public/media/drinks/specials',
    800,
);
...
function copyFiles(inputDir, outputDir, extRegex) {
    ...
    for (const file of files) {
        if (extRegex.test(file)) {
            const newName =
                safeName(path.basename(file, path.extname(file))) +
